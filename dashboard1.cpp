...
{
    qCircular = QPoint(200 / 2 + 40 ,200 / 2);//圆心位置刷新
    qRadius = (200 / 2);//外圆形的半径
...
void Dashboard1::drawDashBoardText(QPainter *painter)
{
    ...
    painter->drawText(QRectF(qCircular.x() - 10,qCircular.y() + 40,80,80),QString::number(qValue));
    float fRadius = 0.0;
    //通过公式计算刻度文字的位置信息。
...
{
    painter->save();
    //保存当前的漆器状态(将状态压入堆栈)。一个save()后面必须跟着一个相应的restore();函数的作用是:展开堆栈。
